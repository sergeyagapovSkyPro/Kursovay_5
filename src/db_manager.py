...
class DBManager:
    ...
    def get_all_vacancies(self):
     ...
     вакансий с указанием названия компании, названия вакансии и зарплаты и ссылки на вакансию"""
        with self.conn:
            with self.conn.cursor() as cursor:
                cursor.execute('SELECT employer_name, vacancy_name, salary_from, url FROM vacancies JOIN employer '
                               'ON employer.employer_id=vacancies.company_id ORDER BY employer_name')
                data = cursor.fetchall()
                print(data)
